...
public class Circle {
    ...
    public void CalculateFactorialCost(int task) {
        ...
        while (isVisited != des) {
            ...
            int penalty = 99999;
            if (adjacentNode.size() == 0 || this.domainLeft.contains(Files.edgesTask.get(task)[isVisited-1][nextNode-1].get(this.edgeIdx[isVisited-1] % Files.edgesTask.get(task)[isVisited-1][nextNode-1].size()).domain)) {
                cost += penalty;
            } else {
