...
public class GeneticMechanism {
    ...
    public static List<Circle> GeneticOperators (Circle c1, Circle c2) {
    	List<Circle> childlist = new ArrayList<Circle>();
        float rand = Mfea4IDPCDU.random.nextFloat();
        Circle child1 = new Circle(c1); 
        Circle child2 = new Circle(c2);

        if((c1.s_factor == c2.s_factor ) || (rand < rmp)) {   //Crossover Condition
            int a, b;
            a = Mfea4IDPCDU.random.nextInt(c1.nodePri.length - 1) + 1;
            b = Mfea4IDPCDU.random.nextInt(c1.nodePri.length - 1) + 1;   
            if (a > b) {
                num = a - b + 1;
                point = b;
            }
            ...
            if(c1.s_factor == c2.s_factor ) {                                   //Parents have the same skill factor
                ...
                rand = Mfea4IDPCDU.random.nextFloat();
                if (rand > 0.5f) {
                    child1.s_factor = c1.s_factor;
                    child2.s_factor = c1.s_factor;
                } 
        ...
        for (int i = 0; i < Files.TASK_NUM; i++) {
            child1.CalculateFactorialCost(i);
            child2.CalculateFactorialCost(i);
        }
    ...
    public static void Mutation(Circle c) {
        ...
        pos1 = Mfea4IDPCDU.random.nextInt(c.nodePri.length);
        pos2 = Mfea4IDPCDU.random.nextInt(c.nodePri.length);
        tmp = c.nodePri[pos1];
        c.nodePri[pos1] = c.nodePri[pos2];
        c.nodePri[pos2] = tmp;

        //Randomly change one edge index
        pos1 = Mfea4IDPCDU.random.nextInt(c.edgeIdx.length);
        c.edgeIdx[pos1] = Mfea4IDPCDU.random.nextInt(PreProcess.css_node_out_edge.length - 1) + 1;
    }
}
